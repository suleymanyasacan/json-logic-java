 ...
 * @author Richard
 */
class SubtractionNegatingOverloadNode extends Node {

    private Node node;

    SubtractionNegatingOverloadNode(Node node) {
        this.node = node;
    }
...
//        return result;
    ...
    private Result tryNegate(Node node, Map<String, Result> data) throws EvaluationException {

        Result result = node.eval(data);

        if (result.isDouble())
            return new Result(-result.getDoubleValue());
